...
//		return eiddList.get(index);
	...
	public int retreveGeneralInfo() throws RemoteException {
		int ret;
		try {
			ret = R_service.getGeneral_Car(generalInfo);
		...
		if(ret == 0) {
			...
			for(int i=0,j=0; i < MAX_BATTARY_SIZE; i++) {
				if(isBattarySet(i)){
					battaryDetialInfo[j] = new EnergyInfoDetailsDriver(R_service, i);
					battaryDetialInfo[j].retriveGeneralBatteryInfo();
					realposition[j] = i;
					j++;
				}
	...
	private int[] realposition = new int[4];
	
	public int getRealBattaryPosition(int index){
		return realposition[index];
	}
